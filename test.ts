...
async function refresh(models: Array<Record<string, unknown>>) {
	...
	return registration.config.refreshModels({
...
describe("catalog reasoning fallback", () => {
	test("enables thinking when Portal marks grok-4.6 as reasoning:false", async () => {
		...
		expect(models).toEqual([expect.objectContaining({ id: "grok-4.6", reasoning: true })]);
	});
	...
	test("keeps explicit non-reasoning and media models disabled", async () => {
		const models = await refresh([
			...
			{ id: "codex-auto-review", reasoning: false },
		]);
